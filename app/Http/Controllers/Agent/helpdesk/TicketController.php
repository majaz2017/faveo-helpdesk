...
class TicketController extends Controller
{
  ...
  public function post_newticket(CreateTicketRequest $request, CountryCode $code, $api = false)
  {
    try {
      ...
      $form_data     = $request->except('name', 'phone', 'email', 'subject', 'body', 'helptopic', '_wysihtml5_mode', '_token', 'mobile', 'code', 'priority', 'first_name', 'last_name', 'sla', 'duedate', 'assignto');
      $auto_response = 0;
      $status        = 1;
      ...
      //create user
      $result = $this->create_user($email, $fullname, $subject, $body, $phone, $phonecode, $mobile_number, $helptopic, $sla, $priority, $source->id, $headers, $help->department, $assignto, $form_data, $auto_response, $status, $latitude, $longitude);
      if ($result[1]) {
          ...
          if ($api != false) {
            ...
            if ($attach != null) {
              try {
                ...
                $storage->saveAttachment($thread->id, $attach);
              } catch (\Exception $ex) {
                loging('attachment', $ex->getMessage());
              }
            }
            return ['ticketnumber' => $ticketId->ticket_number, 'message' => Lang::get('lang.Ticket-created-successfully')];
          }

          return Redirect('newticket')->with('success', Lang::get('lang.Ticket-created-successfully'));
        } else {
          if ($api != false) {
            return response()->json(['success' => Lang::get('lang.Ticket-created-successfully'), 'ticket_number' => $ticketId->ticket_number]);
          }
   ...
   * @return type bool
   */
  public function create_user($emailadd, $username, $subject, $body, $phone, $phonecode, $mobile_number, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $from_data, $auto_response, $status, $latitude, $longitude)
  {
    ...
    if ($checkemail == false) {
      ...
      log::emergency("in create user: user_status " . $user_status);
      ...
      log::emergency("in create user: " . $token);
    ...
    \Event::fire(new \App\Events\ClientTicketFormPost($from_data, $emailadd, $source));
    $ticket_number = $this->check_ticket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $from_data, $status, $latitude, $longitude);
   ...
   * @return type string
   */
  public function check_ticket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $latitude, $longitude)
  {
    ...
    if (isset($read_ticket_number[1])) {
      ...
      if ($find_number->count() > 0) {
        ...
        $ticket_number = $this->createTicket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $latitude, $longitude);
   ...
   * @return type string
   */
  public function createTicket($user_id, $subject, $body, $helptopic, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $latitude, $longitude)
  {
    $ticket_number = '';
